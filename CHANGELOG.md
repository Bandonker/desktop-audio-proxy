...
- Added `createMediaElementController()` for drop-in `<audio>`/`<video>`
  loading, playback, race-safe station switching, and lifecycle cleanup.
- Added optional fail-closed `allowedHosts` enforcement for exact station
  hosts and boundary-safe wildcard CDN subdomains, including redirects.
- Added compilable browser, React, Vue, Electron, Tauri, standalone-host, and
  video/HLS examples plus automated example and README verification gates.
- Added WebKit-aware media-engine detection, typed fallback source selection,
  media-element compatibility defaults, and generic media MIME correction.
- Added real Electron, Tauri 2/WebView2, and Playwright WebKit runtime smoke
  fixtures for environment, proxy, range, and media-load verification.

### Security

- Blocked private and non-public proxy destinations across literal addresses,
  DNS resolution, and redirects; URL credentials are rejected.
- Filtered upstream response headers, disabled credentialed wildcard CORS, and
  redacted target URLs in diagnostic output.
- Bounded the metadata cache and updated/overrode vulnerable production and
  development dependencies; `npm audit` now reports zero known vulnerabilities.
- Made the high-level media controller reject proxy outages by default instead
  of silently returning a direct CORS-prone URL.

### Fixed

- Made generated declaration imports NodeNext-compatible and added a strict
  public-type consumer gate to CI and publishing.
- Repaired main-entry proxy auto-start and adoption of alternate runtime ports.
- Made server start idempotent under concurrent calls and added OS-assigned port support.
- Fixed Vue plugin injection, stale React/Vue async state updates, and React
  telemetry callback preservation.
- Isolated telemetry/debug callbacks so observer failures cannot break media
  operations, and returned defensive debugger log snapshots.
- Made Tauri/Electron optional APIs fail safely when service methods are called
  without browser `window` or `Audio` globals.
- Correctly classify branded iOS browsers as WebKit instead of Chromium.
- Reset queued fetch implementations between Jest tests to prevent
  cross-test contamination after an early failure.
- Prevented partial HLS range responses from being rewritten, and cleared
  upstream media headers before returning JSON errors.
- Stopped hook-owned auto-started servers during React/Vue scope cleanup and
  protected additional async capability/status/metadata state from stale results.
- Made `autoDetect: false` effective and emitted the documented
  `proxy_start`/`proxy_stop` telemetry events for auto-started servers.
- Corrected local-only example bindings, debug script URL lookup, and generated
  frontend demo code; demo servers now reject unsupported HTTP methods.
- Repaired the telemetry dashboard's incomplete module script, counters,
  controls, generated snippets, and browser-safe package loading.

### Changed

- Rebuilt the README and interactive demo around the secure existing-player
  radio workflow, current capability boundaries, and fresh browser evidence.
- Added strict configuration and remote-media URL validation.
- Rejected the previously silent `enableTranscoding: true` no-op and clarified
  that codec decoding remains the host runtime's responsibility.
- Preserved upstream content encoding for byte-correct media streaming.
- Removed the unnecessary post-listen auto-start delay and aligned the default
  upstream user agent with package version 1.1.8.
- Rewrote relative HLS variant, segment, and key URIs through the proxy with a
  bounded manifest size, making advertised M3U8 support work end to end.
- Added focused security, lifecycle, framework, and packaged-runtime regression checks.
- Reduced the published package to supported runtime artifacts, documentation,
  examples, and README assets; builds now clean stale output so the unusable
  legacy `dist/server.js` cannot leak into a local publish, with a package
  payload gate in CI and release workflows.
...
<template>
  ...
  <div v-else-if="error">
    Error: {{ error }} <button @click="retry">Retry</button>
  </div>

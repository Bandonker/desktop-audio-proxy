...
class DemoServer {
  ...
  buildLibrary() {
    return new Promise((resolve, reject) => {
      const command =
        process.platform === 'win32' ? 'cmd.exe' : this.npmCommand;
      const args =
  ...
  startProxyServer() {
    return new Promise((resolve, reject) => {
      ...
      this.proxyProcess.stdout.on('data', data => {
        ...
        const runtimePortMatch = output.match(
          /running on http:\/\/(?:0\.0\.0\.0|localhost):(\d+)/i
        );
        if (runtimePortMatch) {
          this.proxyPort = parseInt(runtimePortMatch[1], 10);
          console.log(`[Proxy] Detected actual proxy port: ${this.proxyPort}`);
        } else {
          const healthPortMatch = output.match(
            /health check:\s*http:\/\/localhost:(\d+)/i
          );
          if (healthPortMatch) {
      ...
      this.proxyProcess.on('close', code => {
        if (!settled) {
          finish(
            reject,
            new Error(
              `Proxy server exited before reporting readiness (code ${code})`
            )
          );
        }
      });
    ...
    return new Promise((resolve, reject) => {
      ...
      this.webServer.on('error', err => {
        if (err.code === 'EADDRINUSE') {
          console.log(
            `⚠️  Port ${this.demoPort} is in use, trying next port...`
          );
          this.demoPort++;
          this.webServer.listen(this.demoPort, 'localhost');
        } else {
  ...
  handleRequest(req, res) {
    ...
    console.log(
      `[WebServer] ${req.method} ${pathname} -> ${filePath} (${mimeType})`
    );
    ...
    fs.readFile(filePath, (err, data) => {
      if (err) {
        console.error(
          `[WebServer] ❌ Server error reading ${filePath}: ${err.message}`
        );
        res.writeHead(500);
  ...
  showInstructions() {
    ...
    console.log(
      `   • Audio Proxy Server: http://localhost:${this.proxyPort || 'unknown'}`
    );
    ...
    console.log('\n For developers:');
    console.log(
      '   • Open browser dev tools to inspect the real implementation'
    );
